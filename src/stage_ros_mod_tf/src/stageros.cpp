...
class StageNode {
 ...
 public:
  ...
  bool cb_reset_srv(std_srvs::Empty::Request& request,
...
const char* StageNode::mapName(const char* name, size_t robotID,
                               Stg::Model* mod) const {
...
const char* StageNode::mapName(const char* name, size_t robotID,
                               size_t deviceID, Stg::Model* mod) const {
...
int StageNode::SubscribeModels() {
  ...
  reset_srv_ =
      n_.advertiseService("reset_positions", &StageNode::cb_reset_srv, this);

  return (0);
}

StageNode::~StageNode() {
  for (std::vector<StageRobot const*>::iterator r = this->robotmodels_.begin();
       r != this->robotmodels_.end(); ++r)
    delete *r;
}

bool StageNode::UpdateWorld() { return this->world->UpdateAll(); }
...
void StageNode::WorldCallback() {
  ...
  for (size_t r = 0; r < this->robotmodels_.size(); ++r) {
    ...
    for (size_t s = 0; s < robotmodel->cameramodels.size(); ++s) {
      ...
      // Translate into ROS message format and publish
      if (robotmodel->image_pubs[s].getNumSubscribers() > 0 &&
      ...
      if (robotmodel->depth_pubs[s].getNumSubscribers() > 0 &&
      ...
      if ((robotmodel->image_pubs[s].getNumSubscribers() > 0 &&
           cameramodel->FrameColor()) ||
          (robotmodel->depth_pubs[s].getNumSubscribers() > 0 &&
           cameramodel->FrameDepth())) {
        ...
        tf::Transform tr = tf::Transform(
            Q, tf::Point(lp.x, lp.y,
                         robotmodel->positionmodel->GetGeom().size.z + lp.z));

        ...
        amp_serial_pkt_control_t c_pkt;                         // Control Data Packet
        	...
        	for(float i = AMP_MIN_VEL; i < AMP_MAX_VEL; i++) {
		   	for(float j = AMP_MIN_ANG; j < AMP_MAX_ANG; j++) {
                ...
                #ifdef DEBUG
                fprintf(fptr1, "buffers before cpy: s_pkt: %hhu c_pkt: %hhu", s_pkt.msg, c_pkt);
                #endif

				memcpy(s_pkt.msg, &c_pkt, sizeof(amp_serial_pkt_control_t));

                #ifdef DEBUG
                fprintf(fptr1, "buffers after cpy: s_pkt: %hhu c_pkt: %hhu", s_pkt.msg, c_pkt);
                #endif
...
amp_err_code_t amp_serial_jetson_rx_pkt(amp_serial_pkt_t * pkt, int bytes) {
    ...
    // Read in the data
    check(sp_nonblocking_read(port, s_buf, sizeof(uint8_t) * bytes), AMP_SERIAL_ERROR_RX);
